...
{
  if ((mode != eSuspend) && (mode != eIaq) && (mode != eUlp) && (mode != ePbaq)) {
...
DFRobot_SGX6410_Gravity_I2C::DFRobot_SGX6410_Gravity_I2C(TwoWire *pWire, uint8_t addr) : _pWire(pWire), _address(addr) {}
...
uint8_t DFRobot_SGX6410_Gravity_I2C::readReg(uint16_t reg, void *data, uint8_t len, eRegType_t regType)
{
  ...
  _pWire->requestFrom(_address, (uint8_t)len);
